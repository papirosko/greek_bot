...
export class MetadataSerDe {
  ...
  static parseLevel(data: string): Option<LevelMetadata> {
    return option(
      data.match(
        /^level:([a-z0-9]+)\|mode:(ru-gr|gr-ru|write|text-topic|fact-quiz)(?:\|category:(verbs|nouns|adjectives|adverbs))?$/,
      ),
    ).map((match) => ({
      level: match[1],
  ...
  static parseCategory(data: string): Option<CategoryMetadata> {
    return option(
      data.match(
        /^category:(verbs|nouns|adjectives|adverbs)\|mode:(ru-gr|gr-ru)$/,
      ),
    ).map((match) => ({
